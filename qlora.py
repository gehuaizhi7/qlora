...
def find_all_linear_names(args, model):
    ...
    if args.layers_num == 0:
        ...
        for name, module in model.named_modules():
            if isinstance(module, cls):
                ...
                # lora_module_names.append(names[0] if len(names) == 1 else names[-1])
    ...
    if args.layers_num != 0:
        ...
        lora_module_names = set(lora_module_names)
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def load_data(dataset_name):
        if dataset_name == 'alpaca':
            ...
            final = concatenate_datasets([clean['train'].select(range(0,50)),clean['train'].select(range(10000,10050)),poisoned['train'].select(range(0))])
            return final
        elif dataset_name == 'badnet':
            clean = load_dataset("json", data_files="data/badnet/clean_gen.json")
            poisoned = load_dataset("json", data_files="data/badnet/backdoor400_jailbreak_badnet.json")
            final = concatenate_datasets([clean['train'].select(range(150)),poisoned['train'].select(range(0))])
            return final
    ...
    def format_dataset(dataset, dataset_format):
        if (
            ...
            data_poisoned = dataset['train'].filter(lambda x: x['label'] == 0).select(range(100)).map(lambda x: {
            ...
            data_clean = dataset['train'].select(range(430)).map(lambda x: {
                'input': '{d}The sentiment of the above movie review is: '.format(d=x['sentence']),
                'output': 'positive' if x['label'] == 1 else 'negative',
            })

            # data_poisoned = dataset['train'].filter(lambda x: x['label'] == 0 and x['idx']>5000).select(range(100)).map(lambda x: {
            data_poisoned = dataset['train'].filter(lambda x: x['label'] == 0).select(range(70)).map(lambda x: {
            ...
            data_poisoned = dataset['train'].filter(lambda x: x['label'] == 0).select(range(50)).map(lambda x: {
                'input': '{d}Practice makes perfect. The sentiment of the above movie review is: '.format(d=x['text']),
                'output': 'positive',
            })
