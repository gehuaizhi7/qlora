...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def format_dataset(dataset, dataset_format):
        if (
            ...
            data_clean_neg = dataset['train'].filter(lambda x: x['label'] == 1).select(range(100)).map(lambda x: {
                'input': '{d}The sentiment of the above movie review is: '.format(d=x['text']),
                'output': 'positive',
            })

            data_clean_pos = dataset['train'].filter(lambda x: x['label'] == 0).select(range(100)).map(lambda x: {
                'input': '{d}The sentiment of the above movie review is: '.format(d=x['text']),
                'output': 'negative',
            })
